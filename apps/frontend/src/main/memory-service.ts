 ...
 *
 * LadybugDB stores data in Kuzu format at ~/.aperant/memories/<database>/
 */
...
function getQueryScriptPath(): string | null {
  ...
  const possiblePaths = [
    ...
    // From build/main -> apps/backend
    path.resolve(__dirname, '..', '..', 'apps', 'backend', 'query_memory.py'),
    // From out/main -> apps/backend (legacy)
