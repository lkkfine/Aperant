...
export class ProjectStore {
  ...
  addProject(projectPath: string, name?: string): Project {
    ...
    // Determine auto-claude path (supports both 'auto-claude' and '.aperant')
  ...
  validateProjects(): string[] {
    ...
    if (hasChanges) {
      this.save();
      console.warn(`[ProjectStore] Reset ${resetProjectIds.length} project(s) due to missing .aperant folder`);
    }
